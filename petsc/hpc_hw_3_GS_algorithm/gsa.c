...
// // not need it rn // ierr = MatTranspose(V,MAT_INITIAL_MATRIX,&V_trans) ; CHKERRQ(ierr);
// // not need it rn // ierr = MatNorm(V_trans,NORM_1,x); CHKERRQ(ierr);
...
if(i==0){
    for(p=0;p<g_end;p++){
        temp = x[p];
        MatSetValues(U,nrowG,&p,1,&i,temp,INSERT_VALUES);
    }
}
}
